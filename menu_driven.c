...
//}
void suggestFriends(Graph* graph, int userIndex) {
    printf("Suggested friends for %s:\n", graph->users[userIndex].username);

    // Array to store distances from the user
    int dist[MAX_USERS];

    // Run Dijkstra's algorithm from the user with a distance constraint of <= 2
    dijkstra(graph, userIndex, dist);

    // Traverse the distances array to find potential friends
    for (int i = 0; i < graph->num_users; ++i) {
        if (dist[i] > 0 && dist[i] <= 2 && !isFriend(graph, userIndex, i)) {
            printf("%s\n", graph->users[i].username);
        }
    }
}

// Modify dijkstra function to include distance constraint
void dijkstra(Graph* graph, int src, int* dist) {
...
// Struct definitions and function prototypes (provided in your code)
...
int main() {
    ...
    file = fopen("/Users/adityadeore/Desktop/SocialNetworkAnalyzer-main/usernames.txt", "r");
    if (file == NULL) {
 ...
 int choice;
    do {
        ...
        printf("4. Optimize Modularity (Conservative)\n");
        ...
        switch (choice) {
            ...
            case 2:
                ...
                break;
            case 3:
                printf("Optimizing modularity (Aggressive)...\n");
                int increased, same;
                aggressiveOptimizationLoop(graph, &increased, &same);
                printf("Number of communities with increased modularity = %d\n", increased);
                printf("Number of communities with increased or same modularity = %d\n", increased + same);
                break;
            case 4:
                printf("Optimizing modularity (Conservative)...\n");
                optimizeModularityConservative(graph);
                printf("Modularity optimized conservatively.\n");
                break;
            case 5:
                printf("Enter user index to find potential users: ");
                int userIndex;
                scanf("%d", &userIndex);
                findPotentialUsers(graph, userIndex);
                break;
            case 6:
                printf("Enter user index to delete: ");
                int deleteUserIndex;
                scanf("%d", &deleteUserIndex);
                removeUser(graph, deleteUserIndex);
                printf("User deleted successfully.\n");
                break;
            case 7:
                printf("Enter user index 1: ");
                int userIndex1;
                scanf("%d", &userIndex1);
                printf("Enter user index 2: ");
                int userIndex2;
                scanf("%d", &userIndex2);
                if (isFriend(graph, userIndex1, userIndex2)) {
                    printf("The two users are friends.\n");
                } else {
            ...
            case 8:
            printf("Enter user index to suggest friends: ");
            int suggestUserIndex;
            scanf("%d", &suggestUserIndex);
            suggestFriends(graph, suggestUserIndex);
            break;
